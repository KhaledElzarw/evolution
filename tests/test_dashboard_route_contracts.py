...
def test_root_dashboard_route_returns_html_shell(monkeypatch):
    ...
    assert '<script src="/static/dashboard.v1.js?v=15"></script>' in html
    assert 'href="/static/dashboard.v1.css?v=15"' in html
...
def test_static_route_serves_dashboard_js_asset(monkeypatch, tmp_path):
    ...
    server = _start_server()
    try:
        status, headers, body = _request(server, "/static/dashboard.v1.js?v=15")
    finally:
        server.shutdown()
        server.server_close()
